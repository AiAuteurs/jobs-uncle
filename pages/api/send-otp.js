    ...
    await resend.emails.send({
      ...
      html: `
        <div style="font-family: Inter, sans-serif; max-width: 480px; margin: 0 auto; padding: 40px 20px; background: #0d0d0d; color: #fff; border-radius: 16px;">
          ...
          <h2 style="font-size: 1.5rem; font-weight: 900; margin: 0 0 12px; letter-spacing: -0.02em;">Your verification code</h2>
          ...
          <div style="background: #1a1a1a; border: 1px solid #333; border-radius: 12px; padding: 28px; text-align: center; margin-bottom: 24px;">
            <div style="font-size: 2.8rem; font-weight: 900; letter-spacing: 0.2em; color: #00D1FF; font-variant-numeric: tabular-nums;">${code}</div>
          </div>
          <p style="color: #555; font-size: 0.78rem; margin: 0; line-height: 1.6;">If you didn't request this, ignore this email. We'll never spam you.</p>
        </div>
      `,
    })
  } catch (err) {
